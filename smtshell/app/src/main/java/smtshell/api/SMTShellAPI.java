...
import com.samsung.SMT.lang.smtshell.AllTheButtons;
...
public class SMTShellAPI {
    ...
    public static void executeCommand(Context context, String cmd) {
        executeCommand(context, cmd, -1, null);
    }

    public static void executeCommand(Context context, String cmd, int requestId, CommandCallback cb) {
        if (cb != null && requestId < 0) {
            throw new IllegalArgumentException("must specify requestId >= 0 when using callback");
        }
    ...
    public static void loadLibrary(Context context, String path) {
        loadLibrary(context, path, -1, null);
    }

    public static void loadLibrary(Context context, String path, int requestId, LoadLibraryCallback cb) {
        if (cb != null && requestId < 0) {
            throw new IllegalArgumentException("must specify requestId >= 0 when using callback");
        }
    ...
    private static Intent createIntent(String action, int requestId) {
    ...
    private static Intent setSender(Context context, Intent intent) {
        int flags = 0;
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            flags = PendingIntent.FLAG_IMMUTABLE;
        }
        intent.putExtra(EXTRA_CALLBACK_PKG, PendingIntent.getBroadcast(context, 0, new Intent(), flags).getIntentSender());
        return intent;
    }
    ...
    public static void killAPI(Context context, InternalCallback cb) {
        Intent intent = createIntent(ACTION_DEACTIVATE, 0);
        if (cb != null) {
            // specify that the request has a sender
            setSender(context, intent);
            // setup receiver
